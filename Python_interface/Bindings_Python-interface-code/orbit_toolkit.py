...
#            self._plot_x_vs_time(rk_positions_all, labels, step_minutes)
    ...
    def visualize_multiple_satellites(self, satellites_data, time_span_hours=6, step_minutes=0.5,
        ...
        """
        Visualize multiple satellites on the same plot.
        
        Args:
            satellites_data (list): List of dictionaries with satellite data.
                Each dictionary should have one of these forms:
                - TLE satellite: {'type': 'tle', 'tle_line1': str, 'tle_line2': str, 'color': str, 'label': str}
                - RK45 satellite: {'type': 'rk45', 'tdm_wildcard': str, 'delta': float, 'delta_error': float, 
                                   'perturbations': dict, 'color': str, 'label': str}
                - Custom satellite: {'type': 'custom', 'positions': list, 'color': str, 'label': str}
            time_span_hours (float): Time span to propagate in hours
            step_minutes (float): Time step in minutes
            show_plot (bool): Whether to show the plot
            save_path (str): Path to save the plot, if None the plot is not saved
        ...
        Returns:
            tuple: (fig, ax) - The figure and axis for the plot
        """
        # Setup the plot
...
class OrbitPropagator(OrbitVisualizer):
    ...
    def compare_orbital_elements(self, tle_line1=None, tle_line2=None, tdm_wildcard=None, delta=100, delta_error=30,
        ...
        if show_plot or save_path:
            ...
            # Plot position and element differences if we have reference trajectory
            if len(trajectories) > 1:
                for traj_idx in range(1, len(trajectories)):
                    traj = trajectories[traj_idx]
                    comparison_name = f"{labels[0]}_vs_{labels[traj_idx]}"
                    
                    # Position magnitude difference plot
                    fig, ax = plt.subplots(1, 1, figsize=(10, 6))
                    ...
                    if save_path:
                        plt.savefig(f"{save_path}_{comparison_name}_position_diff.png", dpi=300, bbox_inches='tight')
                    ...
                    # Semi-major axis difference plot
                    fig, ax = plt.subplots(1, 1, figsize=(10, 6))
                    ...
                    if save_path:
                        plt.savefig(f"{save_path}_{comparison_name}_sma_diff.png", dpi=300, bbox_inches='tight')
                    ...
                    # Optional: Create plots for all orbital element differences
                    element_diff_names = ['Semi-major axis', 'Eccentricity', 'Inclination',
                                          'RAAN', 'Argument of Perigee', 'Mean Anomaly']
                    element_units = ['km', '', 'degrees', 'degrees', 'degrees', 'degrees']
                    
                    for elem_idx, (elem_name, unit) in enumerate(zip(element_diff_names, element_units)):
                        fig, ax = plt.subplots(1, 1, figsize=(10, 6))
                        ax.plot(plot_times, traj['element_diffs'][:, elem_idx])
                        ax.set_title(f"{elem_name} Difference: {labels[0]} vs {labels[traj_idx]}")
                        ax.set_xlabel('Time (hours)')
                        ax.set_ylabel(f'Difference ({unit})' if unit else 'Difference')
                        ax.grid(True)
                        
                        plt.tight_layout()
                        
                        if save_path:
                            safe_elem_name = elem_name.replace(' ', '_').lower()
                            plt.savefig(f"{save_path}_{comparison_name}_{safe_elem_name}_diff.png",
