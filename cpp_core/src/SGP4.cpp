 ...
 /* -----------------------------------------------------------------------------
        ...
        )
    {
        ...
        if (init == 'n')
        {
            ...
            else
            {
                ...
                alfdp = alfdp + dalf;
                betdp = betdp + dbet;
                rec->nodep = fmod(rec->nodep, 2 * M_PI);
                //  sgp4fix for afspc written intrinsic functions
                // nodep used without a trigonometric function ahead
                if ((rec->nodep < 0.0) && (opsmode == 'a'))
                    rec->nodep = rec->nodep + 2 * M_PI;
    ...
    *    vallado, crawford, hujsak, kelso  2006
        ...
        )
    {
        ...
        xnodce = fmod(4.5236020 - 9.2422029e-4 * rec->day, 2 * M_PI);
        stem = sin(xnodce);
        ctem = cos(xnodce);
        ...
        rec->zmol = fmod(4.7199672 + 0.22997150  * rec->day - rec->gam, 2 * M_PI);
        rec->zmos = fmod(6.2565837 + 0.017201977 * rec->day, 2 * M_PI);
    ...
    *    vallado, crawford, hujsak, kelso  2006
        ...
        )
    {
        ...
        if ((rec->inclm < 5.2359877e-2) || (rec->inclm > M_PI - 5.2359877e-2))
            shs = 0.0;
        if (rec->sinim != 0.0)
            shs = shs / rec->sinim;
        ...
        rec->dndt = 0.0;
        theta = fmod(rec->gsto + tc * rptim, 2 * M_PI);
        ...
        if (rec->irez != 0)
        {
            ...
            if (rec->irez == 2)
            {
                ...
                rec->xlamo = fmod(rec->mo + rec->nodeo + rec->nodeo - theta - theta, 2 * M_PI);
            ...
            if (rec->irez == 1)
            {
                ...
                rec->xlamo = fmod(rec->mo + rec->nodeo + rec->argpo - theta, 2 * M_PI);
    ...
    *    vallado, crawford, hujsak, kelso  2006
    ...
    bool sgp4init ( char opsmode,ElsetRec *satrec)
    {
        ...
        if ((satrec->omeosq >= 0.0) || (satrec->no_unkozai >= 0.0))
        {
            ...
            /* --------------- deep space initialization ------------- */
            if ((2 * M_PI / satrec->no_unkozai) >= 225.0)
            {
                satrec->method = 'd';
                satrec->isimp = 1;
                tc = 0.0;
    ...
    *    vallado, crawford, hujsak, kelso  2006
        ...
        )
    {
        ...
        satrec->nodem = fmod(satrec->nodem, 2 * M_PI);
        satrec->argpm = fmod(satrec->argpm, 2 * M_PI);
        xlm = fmod(xlm, 2 * M_PI);
        satrec->mm = fmod(xlm - satrec->argpm - satrec->nodem, 2 * M_PI);
        ...
        if (satrec->method == 'd')
        {
            ...
            {
                xincp = -xincp;
                satrec->nodep = satrec->nodep + M_PI;
                satrec->argpp = satrec->argpp - M_PI;
            }
    ...
    * --------------------------------------------------------------------------- */
    ...
    double gstime(double jdut1)
    {
        ...
        temp = fmod(orbmath::deg2rad(temp) / 240.0, 2 * M_PI); //360/86400 = 1/240, to deg, to rad
